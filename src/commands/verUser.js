...
module.exports = {
  ...
  async run(interaction) {
    ...
    const existdata = res.length !== 0 || res1.length !== 0;
    ...
    if (res.length !== 0) {
      ...
      let first = false;
      res.forEach(data => {
        const verificado = fixMoment(moment(data.verified));
        if (data.type === 0) {
          mET++;
          if (ahora.diff(verificado, 'hours') <= 24) mETH++;
        } else {
          mEF++;
          if (ahora.diff(verificado, 'hours') <= 24) mEFH++;
        }

        if (!first) {
          lastMoment = verificado;
          first = true;
        } else {
          if (lastMoment.diff(verificado, 'second') < 0) {
            lastMoment = verificado;
          }
        }
      });
      ...
      embed.addFields(
        ...
        {
          name: '🕐Ultimo meme Enviado:',
          value: `<t:${lastMoment.unix()}:R>`,
          inline: true,
        },
      );
    }
    if (res1.length !== 0) {
      ...
      const mRA = mRT + mRF;
      const mRAH = mRTH + mRFH;

      embed.addFields(
        {
          name: '📝Memes Revisados:',
          value:
            mRT +
            eNames.approved +
            ' + ' +
